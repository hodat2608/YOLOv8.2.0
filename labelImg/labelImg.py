# # !/usr/bin/env python
# a = 'C:/auto_training/labelImg/classes_link.txt'
# check = 'C:/auto_training/labelImg/check_sheet.txt'
# def Boolean(a, check):
#     global label_want_show
...
#             boolen = False
#     return boolen
# print('ccccc:', Boolean(a, check))
# a = 'C:/auto_training/labelImg/classes_link.txt'
# with open(a, 'r') as f:
#     file_contents = f.read()
#     label_want_show = file_contents
#     print(label_want_show)
# -*- coding: utf-8 -*-
import argparse
import codecs
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        save_format = action(get_format_meta(self.label_file_format)[0], self.change_format,
              'Ctrl+', get_format_meta(self.label_file_format)[1], get_str('changeSaveFormat'), enabled=True)

        close = action(get_str('closeCur'), self.close_file,
                         'Ctrl+W', 'close', get_str('closeCurDetail'))

                
        move_to = action(get_str('deleteImg'), self.move_to,
                        'f', 'close', get_str('saveDetail'))
        copy_to = action(get_str('deleteImg'), self.copy_to,
        'c', 'close', get_str('saveDetail'))
        
        delete_image = action(get_str('deleteImg'), self.delete_image,
                         'Shift+E', 'close', get_str('deleteImgDetail'))
        ...
        copy = action(get_str('dupBox'), self.copy_selected_shape,
                      'Shift+A', 'copy', get_str('dupBoxDetail'), enabled=False)
        ...
        edit = action(get_str('editLabel'), self.edit_label,
                      's', 'edit', get_str('editLabelDetail'),
                      enabled=False)
        ...
        self.actions = Struct(save=save, move_to=move_to, copy_to=copy_to, save_format=save_format, saveAs=save_as, open=open, close=close, resetAll=reset_all, deleteImg=delete_image,
                              ...
                              fileMenuActions=(
                                  open, open_dir, save, move_to,copy_to, save_as, close, reset_all, quit),
                              beginner=(), advanced=(),
        ...
        add_actions(self.menus.file,
                    (open, open_dir, change_save_dir, open_annotation, copy_prev_bounding, self.menus.recentFiles, save, save_as, save_format, close, reset_all, move_to,copy_to, delete_image, quit))
        ...
        self.actions.beginner = (
            open, open_dir, change_save_dir, open_next_image, open_prev_image, verify, save, move_to,copy_to, save_format, None, create, copy, delete, None,
            zoom_in, zoom, zoom_out, fit_window, fit_width)

        self.actions.advanced = (
            open, open_dir, change_save_dir, open_next_image, open_prev_image, save, move_to,copy_to, save_format, None,
    ...
    def move_to(self, _value=False):
        cpath = os.path.join(ustr(self.default_save_dir),'move')
        isExist = os.path.exists(cpath)  
        if not isExist:
            os.makedirs(cpath)
            print('Move to Folder', cpath)
        if self.file_path:
            image_file_name = os.path.basename(self.file_path)
            saved_txt = os.path.join(ustr(self.default_save_dir), image_file_name[:-3] + 'txt')
            saved_img = os.path.join(ustr(self.last_open_dir), image_file_name)
            if os.path.exists(saved_txt):
                shutil.move(saved_txt, cpath + '/' + image_file_name[:-3] + 'txt')
            if os.path.exists(saved_img):
                shutil.move(saved_img, cpath + '/' + image_file_name)
            #self.import_dir_images(self.last_open_dir)
            self.open_next_image()
            # self.cur_img_idx -= 1
            # self.img_count -= 1
    def copy_to(self, _value=False):
        cpath = os.path.join(ustr(self.default_save_dir),'copy')
        isExist = os.path.exists(cpath)  
        if not isExist:
            os.makedirs(cpath)
            print('Copy to Folder', cpath)
        if self.file_path:
            image_file_name = os.path.basename(self.file_path)
            saved_txt = os.path.join(ustr(self.default_save_dir), image_file_name[:-3] + 'txt')
            saved_img = os.path.join(ustr(self.last_open_dir), image_file_name)
            if os.path.exists(saved_txt):
                shutil.copyfile(saved_txt, cpath + '/' + image_file_name[:-3] + 'txt')
            if os.path.exists(saved_img):
                shutil.copyfile(saved_img, cpath + '/' + image_file_name)
            #self.import_dir_images(self.last_open_dir)
            self.open_next_image()
            # self.cur_img_idx -= 1
            # self.img_count -= 1
    ...
    def load_labels(self, shapes):
        ...
        for label, points, line_color, fill_color, difficult in shapes:
            #if label in ['cacbon', 'buichi', 'divat', 'chaudien', 'taychoi', 'ok1', 'ok2', 'ok3']:
            #Khong hien nhan nao thi bo nhan do vao
    ...
    def add_zoom(self, increment=10):
        if increment > 0:
            self.set_zoom(self.zoom_widget.value() + 50)
        elif increment < 0:
            self.set_zoom(self.zoom_widget.value() - 50)
